...
namespace Erp2016.Lib
{
    public class CDormitoryStudentRequest
    {
        ...
        public bool VerifyFirstCancelSheduleChange(int OriginalId, int ChangeType) // Dormitory Cancel=1, Schedule Change=2
        {
            bool result = false;
            int m = Convert.ToInt32(_db.DormitoryCancelScheduleChanges.Where(q => q.OriginalStudentBasicId == OriginalId && q.ChangeType == ChangeType).Count().ToString());
            if (m == 0)
            {
                result = true;
            }
        ...
        public bool VerifyMultiCancelSheduleChange(int OriginalId, int ChangeType) // Dormitory Cancel=1, Schedule Change=2
        {
            bool result = false;
            int m = Convert.ToInt32(_db.DormitoryCancelScheduleChanges.OrderByDescending(q => q.UpdatedDate).FirstOrDefault(q => q.OriginalStudentBasicId == OriginalId && q.ChangeType == ChangeType).RequestStatus.ToString());
            if (m == 1) // 0 = Pending, 1 = Rejected, 2 = Approved
            {
                result = true;
            }
            return result;
